...
from band_selection_16 import band_selection
...
def image_storage(aug_image, copy_gt, training_image_count, path):
    ...
    image_path = os.path.join(path, 'vis_{:020d}.jpg'.format(training_image_count + 1))
    cv2.imwrite(image_path, aug_image)
    text_path = os.path.join(path, 'vis_{:020d}.txt'.format(training_image_count + 1))
...
if __name__ == "__main__":
    ...
    training_dataset_directory = "D:\\temp_hsi_2023\\training\\vis"
    training_videos = os.listdir(training_dataset_directory)
    training_videos.sort()

    validation_dataset_directory = "D:\\temp_hsi_2023\\validation\\vis"
