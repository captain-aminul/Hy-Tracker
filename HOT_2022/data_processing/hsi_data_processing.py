...
def image_storage(aug_image, copy_gt, training_image_count, path):
    ...
    image_path = os.path.join(path, 'img_{:012d}.jpg'.format(training_image_count + 1))
    cv2.imwrite(image_path, aug_image)
    text_path = os.path.join(path, 'img_{:012d}.txt'.format(training_image_count + 1))
...
def validation_image_storage(aug_image, copy_gt, training_image_count, path):
    ...
    image_path = os.path.join(path, 'img_{:015d}.jpg'.format(training_image_count + 1))
    cv2.imwrite(image_path, aug_image)
    text_path = os.path.join(path, 'img_{:015d}.txt'.format(training_image_count + 1))
    h, w, _ = aug_image.shape
    yolo_gt = convert_to_yolo(copy_gt, w, h)
    if os.path.isfile(text_path):
        with open(text_path, 'w') as file:
...
if __name__ == "__main__":
    ...
    training_dataset_directory = "D:/HOT_2022/Train"
    training_videos = os.listdir(training_dataset_directory)
    training_videos.sort()

    validation_dataset_directory = "D:/HOT_2022/Test"
