...
from HOT_2022.inference.band_selection import band_selection
from HOT_2022.inference.classifier import Classifier, set_optimizer, BCELoss
from HOT_2022.inference.data_prov import RegionExtractor, crop_image2
from HOT_2022.inference.evalution_metrices import calculate_euclidean_distance, check_bbox_changes
from HOT_2022.inference.sample_generator import SampleGenerator, overlap_ratio
from HOT_2022.utils.datasets import letterbox
from HOT_2022.utils.experimental import attempt_load
from HOT_2022.inference.new_helper import define_classlist, gen_config, X2Cube, calAUC, xyxy_to_xywh, \
    is_bbox_inside_image
from HOT_2022.inference.sequence_output import SequenceModel
from HOT_2022.models.experimental import attempt_load
from HOT_2022.utils.general import non_max_suppression, scale_coords
from HOT_2022.utils.plots import plot_one_box
from HOT_2022.utils.torch_utils import select_device
...
def initialize_all():
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                       default='weights/hsi_2022.pt', help='model.pt path(s)')
...
def tracking_the_videos(frames, bboxes, class_name):
    ...
    for j in range(1, len(frames)):
        ...
        label = f'{video_name}'
        plot_one_box(copy_tracker, current_image, label=label, color=colors[int(class_name + 1)], line_thickness=1)
...
if __name__=="__main__":
    ...
    # file_name = "../../weights/hot2022_class_name.txt"
    # df = pd.read_csv(file_name, sep="\t", header=None, names=["class_name", "class_number"])
    # class_dict = df.set_index('class_name')['class_number'].to_dict()
    video_dir = "D:/HOT_2022/Test"
    # video_dir = "D:/test_dataset"
    ...
    for video_name in video_list:
        ...
        img_list, gt_list = gen_config(video_dir, video_name, image_format="HSI")
        band_order = band_selection(np.array(Image.open(img_list[0])), np.array(gt_list[0]))
        num_cores = 1  # Use all available CPU cores
        processed_images = Parallel(n_jobs=num_cores)(delayed(process_image)(image_file) for image_file in img_list)
        ...
        print(f"auc: {auc}")
        file = open(os.path.join(result_path, video_name + ".txt"), "x")
        for bb in resultArr:
    ...
    auc = calAUC(all_gts, all_results, video_list )
    print("Total auc:", auc)
